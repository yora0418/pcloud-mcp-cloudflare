...
The query is retained exactly as supplied without trimming or Unicode normalization. Whitespace-only queries, control characters, and unpaired UTF-16 surrogates are rejected before pCloud traversal. Locale-independent lowercase comparison is applied to the query, entry name, and relative path, and the name and path are matched separately so a synthetic separator cannot create a match.
...
Cloudflare Access JWKS retrieval has a 5-second application timeout. pCloud metadata and `getfilelink` calls have 10-second timeouts, while temporary content downloads have a 30-second timeout that also bounds body streaming. Each authenticated MCP request has a 45-second absolute deadline that begins before bounded request-body reading and remains in force through tool execution. Every pCloud fetch uses the shorter of its per-fetch timeout and the remaining request deadline, and the incoming request signal is combined with that timeout so client disconnects cancel request-body reading, stop current work, and prevent another folder request. This requires the tracked Workers `enable_request_signal` compatibility flag. Timeout, abort, and stream failures use generic messages that do not expose abort reasons, credentials, physical paths, or temporary content URLs.
