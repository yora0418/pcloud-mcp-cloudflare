...
>
> **読み取り専用（read-only）は、非公開（private）という意味ではありません。** 認証されたMCPクライアントは、公開範囲内のメタデータやファイル内容を受け取ることができます。`PCLOUD_ROOT_PATH` を使い、Cloudflare Accessで許可した利用者・MCPクライアントへ開示してよい内容だけを公開してください。

pCloudアクセストークンは高価値な認証情報として扱ってください。Gitへコミットしたり、Issue、スクリーンショット、ログ、AIチャットなどへ貼り付けたりしないでください。
